...
# Usage: python3 scripts/fetch-landmark-photos.py

import json, re, time, urllib.request, urllib.parse, os

OUT = os.path.join(os.path.dirname(__file__), '../backend/public/data/landmarks.js')
...
SEED = [
    ...
    {"name": "Charles Bridge",                  "iso": 203, "wiki": "Charles_Bridge"},
    ...
    {"name": "Tallinn Old Town",                "iso": 233, "wiki": "Tallinn_Old_Town"},
    {"name": "Cathedral of Saint Sava",         "iso": 688, "wiki": "Cathedral_of_Saint_Sava"},
    ...
    {"name": "Parliament Hill",                 "iso": 124, "wiki": "Parliament_Hill"},
    {"name": "Tikal",                           "iso": 320, "wiki": "Tikal"},
    {"name": "Cartagena de Indias",             "iso": 170, "wiki": "Cartagena_de_Indias"},
...
def main():
    ...
    for entry in SEED:
        ...
        photo_url = fetch_thumbnail(entry['wiki'])
        if photo_url:
            results.append({"name": entry["name"], "iso": entry["iso"], "photo_url": photo_url})
            print("OK")
        else:
            failed.append(entry)
            print("FAILED (no thumbnail)")
        time.sleep(0.2)
    ...
    print(f"\nWrote {len(results)} landmarks to {os.path.abspath(OUT)}")

    if failed:
        print(f"\nFailed ({len(failed)}) — fix wiki slug in the script and re-run:")
        for e in failed:
            print(f'  iso:{e["iso"]}  wiki:"{e["wiki"]}"  name:"{e["name"]}"')
